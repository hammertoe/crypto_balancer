...
class Attempt():
    def __init__(self, portfolio, orders=None, total_fee=0.0, depth=0, pairs_processed=None):
...
class SimpleBalancer():
    ...
    def balance(self, initial_portfolio, exchange, accuracy=False):
        ...
        while todo:
            attempt = todo.pop()
            diffs = self.permute_differences(attempt.portfolio.differences_quote)

            if not diffs or attempt.depth > 6:
                if attempt.portfolio.balance_rmse < initial_portfolio.balance_rmse \
            ...
            for pos, neg in diffs:
                ...
                min_trade_amount_quote = exchange.limits[trade_pair]['cost']['min']
        ...
        if accuracy:
            sort_key = lambda x: (x.portfolio.balance_rmse, x.total_fee, x.orders)
        else:
            sort_key = lambda x: (x.total_fee, x.portfolio.balance_rmse, x.orders)
